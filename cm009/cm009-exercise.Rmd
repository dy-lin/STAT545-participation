...
```{r}
# negative sign can go outside or inside of the vector
lotr_wide %>% 
...
```{r}
# group_by_all: group by every variable
# rnorm: normal distribution
# unnest: unravel the 'list'
n <- 100
mu <- c(-5, 0, 5)
sd <- c(1, 3, 10)
tibble(mu = mu, sd = sd) %>% 
...
```{r}
# expand gives all possible combinations of inputs, even if not present in the dataset, and only includes the columns given, with no NAs
